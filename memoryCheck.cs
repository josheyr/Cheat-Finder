...
namespace fSS
{
    class memoryCheck
    {
        ...
        // finally...
        public static void run(Form1 frm)
        {
            ...
            while (proc_min_address_l < proc_max_address_l)
            {
                ...
                PAGE_READWRITE && mem_basic_info.State == MEM_COMMIT)
                {
                    ...
                    frm.Invoke(new Action(() => frm.setProgressBar(1)));

                    for (int i = 0; i < mem_basic_info.RegionSize; i++)
                    {
                        if((char)buffer[i] == ' ' || (char)buffer[i] == ' ') { } else { 
...
(mem_basic_info.BaseAddress + i).ToString("X"), (char)buffer[i]);
            ...
            frm.Invoke(new Action(() => frm.incrementProgressBar()));
